...
def create_obj():
    ...
    if not request.get_json():
        return make_response(jsonify({"error": "Si JSON"}), 400)
    if 'name' not in request.get_json():
        return make_response(jsonify({"error": "Jina linakosekana"}), 400)
    js = request.get_json()
    obj = State(**js)
    obj.save()
...
def post_method(state_id):
    """Sasisha kwa kutumia njia ya POST"""
    if not request.get_json():
        return make_response(jsonify({"error": "Si JSON"}), 400)
